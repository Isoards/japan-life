  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button onClick={() => setShowForm(!showForm)} className="ml-auto px-4 py-1.5 rounded-lg text-sm font-medium bg-purple-500/20 text-purple-300 hover:bg-purple-500/30 transition-colors">
          + 추가
        </button>
      </div>
        ...
        {filtered.map((note) => {
          ...
          return (
            <div key={note.id} className="flex items-start gap-3 p-3 rounded-lg border border-white/10 bg-white/5 group">
              ...
              <button onClick={() => handleDelete(note.id)} className="text-gray-500 hover:text-red-400 transition-colors p-1 opacity-0 group-hover:opacity-100 shrink-0" title="삭제">
