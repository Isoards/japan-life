...
export default async function DishDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const cookedItem = overview.cookedDishes.items.find((item) => item.dishId === id);
  ...
  return <div className="space-y-7">
    ...
    <div className="flex flex-col items-start justify-between gap-3 rounded-xl border border-sky-400/15 bg-sky-500/5 p-4 sm:flex-row sm:items-center"><div><p className="font-medium text-white">이 요리를 만들어 보셨나요?</p><p className="mt-1 text-xs text-gray-500">체크하면 요리 찾기의 ‘해본 요리’에서 모아볼 수 있어요.{cookedItem ? ` · 최근 체크 ${new Date(cookedItem.cookedAt).toLocaleDateString("ko-KR")}` : ""}</p></div><CookedButton dishId={dish.id} cooked={Boolean(cookedItem)} /></div>
