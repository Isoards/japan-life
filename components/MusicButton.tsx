...
export default function MusicButton({
  ...
  const spotifyUri = `spotify:search:${encodeURIComponent(searchQuery)}`;
  const spotifyWebUrl = `https://open.spotify.com/search/${encodeURIComponent(searchQuery)}`;
  ...
  const handleAppOpen = useCallback(
    (e: React.MouseEvent) => {
      ...
      // Try URI scheme → opens Spotify app (desktop & mobile)
      window.location.href = spotifyUri;

      // Fallback: if app didn't open after 1.5s, open web
